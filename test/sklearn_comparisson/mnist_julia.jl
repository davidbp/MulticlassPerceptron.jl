...
function load_MNIST( ;array_eltype::DataType=Float32, verbose::Bool=true)
    ...
    train_imgs = MLDatasets.MNIST(:train).features                 # size(train_imgs) -> (28, 28, 60000)
    test_imgs = MLDatasets.MNIST(:test).features                   # size(test_imgs) -> (28, 28, 10000)
    ...
    train_x    = reshape(train_imgs, 784, 60000)
    test_x    = reshape(test_imgs, 784, 10000)


    ## Prepare data
    #train_y = MNIST.labels(:train) .+ 1;
    #test_y  = MNIST.labels(:test)  .+ 1;
    train_y = MNIST(:train).targets .+ 1;
    test_y = MNIST(:test).targets .+ 1;
